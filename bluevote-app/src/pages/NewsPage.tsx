...
const ELECTION_NEWS = [
  { id: 1, title: "2026 Governor Races Heat Up Across 36 States", category: "National", date: "Jul 11, 2026", summary: "With primaries concluded in most states, the general election battle lines are drawn. BlueVote is tracking every race with real-time data.", readTime: "4 min" },
  ...
  { id: 6, title: "How Small Donations Are Reshaping Campaign Finance", category: "Analysis", date: "Jul 6, 2026", summary: "The rise of micro-donations through platforms like BlueVote is changing how campaigns fund their operations. Here's the data.", readTime: "6 min" },
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-blue-300/60 mt-1">Stay informed on races, policy, and platform updates</p>
      </div>

      {/* Breaking Banner */}
      <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
        <div className="flex items-center gap-3">
          <span className="px-2 py-0.5 bg-blue-500 text-white text-xs font-bold rounded uppercase tracking-wider">Live</span>
      ...
      <div className="flex flex-wrap gap-2">
        {CATEGORIES.map(cat => (
          <button key={cat} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${cat === "All" ? "bg-blue-500 text-white" : "bg-white/5 text-white/60 hover:bg-white/10 hover:text-white"}`}>
            {cat}
          </button>
        ))}
      </div>
      ...
      {/* Featured Article */}
      <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-500/20 to-blue-600/5 border border-blue-500/20">
        <span className="px-2 py-0.5 bg-blue-500/20 text-blue-400 text-xs font-bold rounded uppercase tracking-wider">Featured</span>
        <h2 className="text-2xl font-bold text-white mt-3">{ELECTION_NEWS[0].title}</h2>
        <p className="text-blue-200/60 mt-2">{ELECTION_NEWS[0].summary}</p>
        <div className="flex items-center gap-4 mt-4 text-xs text-blue-300/40">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {ELECTION_NEWS.slice(1).map(article => (
          <div key={article.id} className="p-5 rounded-xl bg-white/5 border border-white/10 hover:border-blue-500/30 transition-colors cursor-pointer group">
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2 py-0.5 bg-white/5 text-blue-400 text-xs font-medium rounded">{article.category}</span>
              <span className="text-xs text-blue-300/40">{article.readTime}</span>
            </div>
            <h3 className="text-lg font-semibold text-white group-hover:text-blue-400 transition-colors">{article.title}</h3>
            <p className="text-sm text-white/40 mt-2 line-clamp-2">{article.summary}</p>
            <div className="text-xs text-blue-300/30 mt-3">{article.date}</div>
          </div>
        ))}
      </div>
            ...
            {activity.slice(0, 8).map((item, i) => (
              <div key={i} className="flex items-start gap-3 text-sm">
                ...
                <div>
                  ...
                  <span className="text-blue-300/30 ml-2 text-xs">{item.createdAt}</span>
                </div>
              </div>
            ))}
          </div>
