      ...
      {activeTab === "dashboard" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Total Sent", value: mockStats.totalSent.toLocaleString(), color: "text-blue-400" },
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <div className="flex items-center gap-4 mb-3">
              <div className="flex-1 bg-white/10 rounded-full h-4"><div className="bg-blue-500 h-4 rounded-full" style={{ width: `${(mockStats.todaySent / 500) * 100}%` }} /></div>
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map(c => (
              <button key={c.id} onClick={() => setScriptFilter(c.id)}
                className={"px-3 py-1.5 rounded-lg text-sm transition-colors " + (scriptFilter === c.id ? "bg-blue-600 text-white" : "bg-white/5 text-white/50 hover:bg-white/10")}>
                {c.label} ({c.count})
              </button>
            ))}
          </div>
          {scriptTemplates.filter(s => scriptFilter === "all" || s.category === scriptFilter).map(script => (
            <div key={script.id} className={"bg-white/5 border rounded-xl p-5 cursor-pointer transition-colors " + (selectedScript === script.id ? "border-blue-400" : "border-white/10 hover:border-white/20")} onClick={() => setSelectedScript(selectedScript === script.id ? null : script.id)}>
              ...
              {selectedScript === script.id && (
                <div className="mt-4 flex gap-3">
                  <button className="bg-blue-600 hover:bg-blue-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">Use This Script</button>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-400 h-32 resize-none" defaultValue={scriptTemplates[0].body} />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-semibold transition-colors">Start Campaign</button>
        ...
        <div className="space-y-4">
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="bg-blue-600 hover:bg-blue-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">Add Volunteer</button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{mockVolunteers.map(v => (
                  <tr key={v.name} className="border-t border-white/5">
                    ...
                    <td className="py-3"><button className="text-blue-400 hover:text-blue-300 text-xs">Manage</button></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
