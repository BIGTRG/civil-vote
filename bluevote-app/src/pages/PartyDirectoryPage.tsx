      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2">
            Democratic Party <span className="text-blue-400">Directory</span>
          </h1>
          <p className="text-white/50 text-sm sm:text-base">
            Federal and state-level Democratic Party organizations across the United States
          </p>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-blue-300">{stats.federal}</div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center hidden sm:block">
              <div className="text-2xl font-bold text-blue-200">{stats.state}</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {federalOrgs.map(org => (
                <a
                  ...
                  className="group bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-blue-500/10 hover:border-blue-500/30 transition-all"
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      ...
                      {org.abbreviation && (
                        <span className="inline-block mt-1 px-2 py-0.5 bg-blue-500/10 text-blue-400 text-xs rounded-full font-medium">
                          {org.abbreviation}
                        </span>
                      )}
                    </div>
                    <svg className="w-4 h-4 text-white/20 group-hover:text-blue-400 transition-colors shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {stateOrgs.sort((a, b) => (a.state ?? "").localeCompare(b.state ?? "")).map(org => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center justify-between bg-white/5 border border-white/10 rounded-lg px-4 py-3 hover:bg-blue-500/10 hover:border-blue-500/30 transition-all"
                >
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-white group-hover:text-blue-300 transition-colors text-sm truncate">
                      {org.state}
                    </div>
                    <div className="text-[11px] text-white/30 truncate">{org.name}</div>
                  </div>
                  <svg className="w-4 h-4 text-white/20 group-hover:text-blue-400 transition-colors shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                  </svg>
                </a>
              ))}
