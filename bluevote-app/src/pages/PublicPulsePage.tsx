...
export function PublicPulsePage() {
  ...
  if (!overview) {
    return <div className="text-center py-16 text-blue-300/40">Loading Public Pulse...</div>;
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-blue-300/60 mt-1">Real-time verified voter sentiment -- not polls, real pledges</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Democrat Pledges", value: overview.partyTotals?.democrat?.pledges ?? 0, color: "text-blue-400" },
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-center text-sm font-bold text-white transition-all ${
                        c.party === "democrat" ? "bg-blue-500" : c.party === "republican" ? "bg-red-500" : "bg-gray-500"
                      }`}
                      style={{ width: `${pct}%` }}
                    >
                  ...
                  <div key={c.candidateId} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-blue-300/40 capitalize">{c.party}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-white">{c.pledgeCount.toLocaleString()}</div>
                      <div className="text-xs text-blue-300/40">${c.totalRaised.toLocaleString()}</div>
                    </div>
                  </div>
                ))}
              </div>
