...
export function RaceDetailPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Back link */}
      <Link to="/races" className="text-blue-400 hover:text-blue-300 text-sm flex items-center gap-1">
      ...
      <div>
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">{race.state}</span>
        </div>
        <h1 className="text-3xl font-bold text-white">{race.title}</h1>
        {race.description && <p className="text-blue-300/50 mt-2">{race.description}</p>}
        <div className="flex gap-8 mt-4">
          <div>
            <span className="text-2xl font-bold text-white">{race.totalPledges.toLocaleString()}</span>
            <span className="text-blue-300/40 text-sm ml-2">total pledges</span>
          </div>
          <div>
            <span className="text-2xl font-bold text-blue-400">${race.totalRaised.toLocaleString()}</span>
            <span className="text-blue-300/40 text-sm ml-2">total raised</span>
          </div>
        </div>
      </div>
            ...
            {race.pulse.map((p, i) => (
              <div
                ...
                className={`flex items-center justify-center text-xs font-bold text-white ${
                  p.party === "democrat" ? "bg-blue-500" : p.party === "republican" ? "bg-red-500" : "bg-gray-500"
                }`}
          ...
          <div className="flex flex-wrap gap-4">
            {race.pulse.map(p => (
              <div key={p.candidateId} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-blue-300/40">{p.pledgeCount.toLocaleString()} pledges</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={candidate._id} className="p-6 rounded-2xl bg-white/5 border border-white/10">
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold text-white ${
                    candidate.party === "democrat" ? "bg-blue-500/30" : candidate.party === "republican" ? "bg-red-500/30" : "bg-gray-500/30"
                  }`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        candidate.party === "democrat" ? "bg-blue-500/20 text-blue-400" :
                    ...
                    <p className="text-blue-300/50 text-sm mb-4">{candidate.bio}</p>

                    {/* Positions */}
                    <div className="grid sm:grid-cols-2 gap-2 mb-4">
                      {candidate.positions.map(pos => (
                        <div key={pos.issue} className="p-3 rounded-lg bg-white/5">
                          <div className="text-xs font-medium text-blue-400 mb-1">{pos.issue}</div>
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                      <div className="text-sm">
                        ...
                        <span className="text-blue-300/40 ml-1">pledges ({pct}%)</span>
                      </div>
                      <div className="text-sm">
                        <span className="font-bold text-blue-400">${candidate.totalRaised.toLocaleString()}</span>
                        <span className="text-blue-300/40 ml-1">raised</span>
                      </div>
                      <button
                        onClick={() => {
                        ...
                        className="ml-auto px-5 py-2 bg-blue-500 hover:bg-blue-400 text-white text-sm font-medium rounded-lg transition-colors"
                      >
                        Pledge Support
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0f2140] border border-white/10 rounded-2xl p-8 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-blue-300/60 mb-1">Pledge Amount ($)</label>
                <input
                  type="number"
                  min="5"
                  value={pledgeAmount}
                  onChange={e => setPledgeAmount(e.target.value)}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white text-lg focus:outline-none focus:border-blue-400/50"
                />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-400 transition-colors"
                >
                  Confirm Pledge
                </button>
              </div>
