  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-blue-300/60 mt-1">Find races in your state and pledge your support</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <select
          ...
          className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-blue-400/50"
        >
      ...
      <div className="grid gap-4">
        {filtered.map((race) => (
          <Link
            key={race._id}
            to={`/race/${race._id}`}
            className="block p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-blue-400/30 transition-all hover:-translate-y-0.5"
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex-1">
                ...
                {race.description && (
                  <p className="text-blue-300/50 text-sm">{race.description}</p>
                )}
              </div>
              <div className="flex gap-6 md:gap-8">
                <div className="text-center">
                  <div className="text-2xl font-bold text-white">{race.totalPledges.toLocaleString()}</div>
                  <div className="text-xs text-blue-300/40">Pledges</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-400">${race.totalRaised.toLocaleString()}</div>
                  <div className="text-xs text-blue-300/40">Raised</div>
                </div>
              </div>
            </div>
          </Link>
