...
export function PushNotificationsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {/* Push enable banner */}
      {!pushEnabled && (
        <div className="bg-blue-600/10 border border-blue-400/30 rounded-xl p-5 mb-6 flex items-center justify-between flex-wrap gap-4">
          <div>
          ...
          <button onClick={enablePush} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-lg font-medium transition-colors shrink-0">Enable Push</button>
        </div>
      )}
          ...
          {notifChannels.map(ch => (
            <div key={ch.id} className="flex items-center justify-between p-4 bg-white/5 rounded-lg">
              ...
              <button onClick={() => ch.id === "push" && !pushEnabled ? enablePush() : setChannels(prev => ({ ...prev, [ch.id]: !prev[ch.id] }))}
                className={"w-12 h-7 rounded-full transition-colors relative " + (channels[ch.id] ? "bg-blue-600" : "bg-white/20")}>
          ...
          {notifTypes.map(nt => (
            <div key={nt.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors">
              ...
              <button onClick={() => setPrefs(prev => ({ ...prev, [nt.id]: !prev[nt.id] }))}
                className={"w-10 h-6 rounded-full transition-colors relative " + (prefs[nt.id] ? "bg-blue-600" : "bg-white/20")}>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="time" value={quietStart} onChange={e => setQuietStart(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-400" />
          </div>
          <div>
            <label className="block text-sm text-white/50 mb-1">End</label>
            <input type="time" value={quietEnd} onChange={e => setQuietEnd(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-400" />
          </div>
        </div>
      </div>

      <button className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-semibold transition-colors">Save Preferences</button>
    </div>
  );
}
