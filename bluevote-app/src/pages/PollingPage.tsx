...
function RatingBadge({ rating }: { rating: string }) {
  const colors: Record<string, string> = {
    "Safe D": "bg-blue-600/30 text-blue-300 border-blue-500/30",
    "Likely D": "bg-blue-500/25 text-blue-300 border-blue-400/30",
    "Lean D": "bg-blue-400/20 text-blue-300 border-blue-300/30",
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-blue-300/50 mt-1">Real-time polling, race ratings, and FEC campaign finance data</p>
      </div>

      {/* Data sources banner */}
      <div className="flex flex-wrap gap-2">
        {["FEC.gov", "Cook Political Report", "Sabato Crystal Ball", "Fox News", "Emerson College", "SurveyUSA", "Marist"].map(src => (
          <span key={src} className="px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-300 text-xs font-medium">
            {src}
          </span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
            ...
            <div className="text-blue-300/40 text-xs mt-1">{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      {senateRatings.length > 0 && (
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
          ...
          <p className="text-blue-300/40 text-sm mb-4">Cook Political Report -- Updated July 2026</p>
          
          <div className="space-y-1">
            <div className="grid grid-cols-3 text-xs font-semibold text-blue-300/50 uppercase tracking-wider px-3 pb-2 border-b border-white/5">
              <span>State</span>
              <span>Office</span>
            ...
            {senateRatings.sort((a, b) => {
              ...
              <div key={i} className="grid grid-cols-3 items-center px-3 py-2.5 rounded-lg hover:bg-white/5 transition-colors">
                ...
                <span className="text-blue-300/60 text-sm">U.S. Senate</span>
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
          ...
          <div className="space-y-1">
            ...
            {govRatings.sort((a, b) => {
              ...
              <div key={i} className="grid grid-cols-3 items-center px-3 py-2.5 rounded-lg hover:bg-white/5 transition-colors">
                <span className="text-white font-medium text-sm">{r.state}</span>
                <span className="text-blue-300/60 text-sm">Governor</span>
                <span className="text-right"><RatingBadge rating={r.rating} /></span>
              </div>
            ))}
          </div>
      ...
      {topFundraisers && topFundraisers.length > 0 && (
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
          ...
          <p className="text-blue-300/40 text-sm mb-4">Real campaign finance filings from FEC.gov</p>
          
          <div className="space-y-3">
            {topFundraisers.map((c, i) => {
              const maxRaised = topFundraisers[0]?.totalRaised || 1;
              const pct = (c.totalRaised / maxRaised) * 100;
              const partyColor = c.party === "democrat" ? "bg-blue-500" : c.party === "republican" ? "bg-red-500" : "bg-purple-500";
              const partyBg = c.party === "democrat" ? "bg-blue-500/10" : c.party === "republican" ? "bg-red-500/10" : "bg-purple-500/10";
              return (
                <div key={c._id} className={`p-3 rounded-lg ${partyBg} border border-white/5`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-blue-300/40 text-xs ml-2">
                  ...
                  {(c.totalSpent || c.cashOnHand) && (
                    <div className="flex gap-4 mt-1.5 text-xs text-blue-300/40">
