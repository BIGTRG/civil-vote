  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-blue-300/60 mt-1">Platform engagement metrics and voter intelligence</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={m.label} className="p-5 rounded-xl bg-white/5 border border-white/10">
            ...
            <div className="text-sm text-blue-400 mt-1">{m.label}</div>
            <div className="text-xs text-blue-300/30 mt-0.5">{m.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
          ...
          <div className="space-y-4">
            {[
              { label: "State Races", count: stateRaces, color: "bg-blue-500", pct: races?.length ? Math.round((stateRaces / races.length) * 100) : 0 },
              { label: "Federal Races", count: federalRaces, color: "bg-blue-400", pct: races?.length ? Math.round((federalRaces / races.length) * 100) : 0 },
              { label: "Local Races", count: localRaces, color: "bg-blue-300", pct: races?.length ? Math.round((localRaces / races.length) * 100) : 0 },
            ].map(item => (
              <div key={item.label}>
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
          ...
          <div className="space-y-4">
            {[
              ...
              { label: "In Progress", count: promiseStats?.byStatus?.in_progress ?? 0, color: "bg-blue-500" },
          ...
          <div className="space-y-3">
            {topStates.map(([state, pledges], i) => (
              <div key={state} className="flex items-center gap-4">
                <span className="text-xs text-blue-300/40 w-5">{i + 1}</span>
          ...
          {activity?.slice(0, 10).map((item, i) => (
            <div key={i} className="flex items-start gap-4">
              ...
              <div className="flex-1 -mt-0.5">
                ...
                <div className="text-xs text-blue-300/30 mt-0.5">{item.createdAt}</div>
              </div>
            </div>
          ))}
        </div>
