      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              {exportTypes.map(et => (
                <div key={et.id} onClick={() => toggle(et.id)}
                  className={"flex items-start gap-4 p-4 rounded-lg border cursor-pointer transition-colors " + (selectedExports.includes(et.id) ? "bg-blue-500/10 border-blue-400/50" : "bg-white/5 border-white/10 hover:border-white/20")}>
                  <div className={"w-5 h-5 rounded border-2 mt-0.5 flex items-center justify-center shrink-0 " + (selectedExports.includes(et.id) ? "bg-blue-500 border-blue-500" : "border-white/30")}>
        ...
        <div className="space-y-4">
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {(["csv","pdf","json"] as const).map(f => (
                    <button key={f} onClick={() => setFormat(f)} className={"flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors " + (format === f ? "bg-blue-600 text-white" : "bg-white/10 text-white/50")}>{f.toUpperCase()}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm text-white/50 mb-1">Date Range</label>
                <select value={dateRange} onChange={e => setDateRange(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-400">
              ...
              <div>
                ...
                <select value={stateFilter} onChange={e => setStateFilter(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-400">
              ...
              <button onClick={handleExport} disabled={selectedExports.length === 0 || exporting}
                className={"w-full py-3 rounded-lg font-semibold transition-colors " + (selectedExports.length === 0 ? "bg-white/10 text-white/30 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-500 text-white")}>
            ...
            <div className="space-y-3">
              {recentExports.map(ex => (
                <div key={ex.name} className="flex items-center justify-between text-sm">
                  ...
                  <button className="text-blue-400 hover:text-blue-300 text-xs shrink-0 ml-2">Download</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            ...
            <button className="w-full mt-3 text-sm text-blue-400 hover:text-blue-300">Schedule New Report</button>
          </div>
        </div>
      </div>
    </div>
