  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-blue-300/60 mt-1">The Carfax for politicians -- every promise recorded, tracked, and verified</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            ...
            { label: "In Progress", value: stats.byStatus.in_progress, color: "text-blue-400" },
            ...
            <div key={s.label} className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
              ...
              <div className="text-xs text-blue-300/40 mt-1">{s.label}</div>
            </div>
          ))}
        </div>
      )}
            ...
            <div key={promise._id} className="p-6 rounded-2xl bg-white/5 border border-white/10">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                <div className="flex-1">
                  ...
                  <p className="text-blue-300/50 text-sm">{promise.description}</p>
                </div>
                <div className="text-right shrink-0">
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${
                      promise.candidateParty === "democrat" ? "bg-blue-500/30" : "bg-red-500/30"
                    }`}>
                      {promise.candidateName.split(" ").map(n => n[0]).join("")}
                    </div>
                    <div>
                      <div className="text-sm font-medium text-white">{promise.candidateName}</div>
                      <div className="text-xs text-blue-300/40">{promise.raceName}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {promise.evidence.length > 0 && (
                <div className="border-t border-white/5 pt-4">
                  <div className="text-xs text-blue-300/40 mb-2">Evidence Chain</div>
                  ...
                  <div className="mt-3 text-xs font-mono text-blue-300/30">
                    Hash: {promise.hash}
                  </div>
                </div>
              )}
