  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Active Tests", value: activeTests.filter(t => t.status === "running").length.toString(), color: "text-blue-400" },
            ...
            <div key={test.id} className="bg-white/5 border border-white/10 rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={"px-3 py-1 rounded-full text-xs font-medium " + (test.status === "running" ? "bg-blue-500/20 text-blue-400" : "bg-green-500/20 text-green-400")}>{test.status}</span>
                </div>
              </div>
                    ...
                    <div key={i} className={"p-4 rounded-lg border " + (isWinner ? "bg-green-500/5 border-green-400/30" : "bg-white/5 border-white/10")}>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex-1 bg-white/10 rounded-full h-2">
                          <div className={"h-2 rounded-full " + (isWinner ? "bg-green-500" : "bg-blue-500")} style={{ width: `${(v.rate / maxRate) * 100}%` }} />
                        </div>
      ...
      {activeTab === "create" && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="e.g., Homepage CTA Color" className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-400" />
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <button className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-semibold transition-colors">Launch Test</button>
