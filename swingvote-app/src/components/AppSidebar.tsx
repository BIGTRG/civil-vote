...
const navItems = [
  ...
  },
  {
    group: "COMPARE",
    items: [
      { label: "Side by Side", path: "/compare", icon: "M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" },
...
export function AppSidebar({ onNavigate }: { onNavigate?: () => void } = {}) {
  ...
  return (
    <aside className="w-[240px] h-screen bg-[#0f0a14] border-r border-purple-900/20 flex flex-col flex-shrink-0">
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 py-5 border-b border-purple-900/20">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-600 to-amber-500 flex items-center justify-center text-white font-bold text-sm">SV</div>
        <span className="text-lg font-bold text-white">Swing<span className="text-amber-400">Vote</span></span>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-6 overflow-y-auto">
        {navItems.map((group) => (
          <div key={group.group}>
            <div className="text-purple-300/30 text-xs font-semibold tracking-wider px-3 mb-2">{group.group}</div>
            <div className="space-y-0.5">
              {group.items.map((item) => {
                const isActive = location.pathname === item.path || (item.path === "/races" && location.pathname.startsWith("/race"));
                return (
                  <button key={item.path} onClick={() => go(item.path)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-purple-600/20 text-amber-400" : "text-purple-200/50 hover:bg-white/5 hover:text-white"}`}>
      ...
      {/* Bottom */}
      <div className="px-3 py-4 border-t border-purple-900/20">
        <button onClick={() => go("/settings")}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-purple-200/50 hover:bg-white/5 hover:text-white transition-colors">
