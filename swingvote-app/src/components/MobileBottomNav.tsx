...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#0a0612]/95 backdrop-blur-lg border-t border-purple-500/10 md:hidden safe-bottom">
      <div className="flex items-center justify-around h-16">
        {tabs.map((tab) => {
          const active = pathname === tab.href || pathname.startsWith(tab.href + "/");
          return (
            <Link key={tab.href} to={tab.href} className={`flex flex-col items-center gap-0.5 px-2 py-1 ${active ? "text-amber-400" : "text-white/40"}`}>
