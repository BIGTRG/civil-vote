...
export function CookieConsent() {
  ...
  useEffect(() => {
    const consent = localStorage.getItem("swingvote_cookie_consent");
    if (!consent) {
  ...
  const accept = () => {
    localStorage.setItem("swingvote_cookie_consent", "accepted");
    setVisible(false);
  };

  const decline = () => {
    localStorage.setItem("swingvote_cookie_consent", "declined");
    setVisible(false);
  };
    ...
    <div className="fixed bottom-20 md:bottom-4 left-4 right-4 md:left-auto md:right-4 md:max-w-md z-[60] animate-in slide-in-from-bottom duration-300">
      <div className="bg-[#1a1a2e] border border-white/10 rounded-xl p-4 shadow-2xl">
        <p className="text-white/80 text-sm mb-3">
          ...
          See our <Link to="/privacy" className="text-purple-400 underline">Privacy Policy</Link> for details.
        </p>
