...
function getSessionId() {
  let id = sessionStorage.getItem("sv_cart_session");
  if (!id) {
    id = crypto.randomUUID();
    sessionStorage.setItem("sv_cart_session", id);
  }
  return id;
}
...
export function StorePage() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">
            Swing<span className="text-amber-400">Store</span>
          </h1>
          <p className="text-white/50 mt-1">Independent gear for independent thinkers</p>
        </div>
        <button
          onClick={() => setCartOpen(!cartOpen)}
          className="relative flex items-center gap-2 px-4 py-2.5 bg-purple-500/15 hover:bg-purple-500/25 border border-purple-400/20 rounded-xl text-purple-400 transition-all"
        >
      ...
      {/* Cart drawer */}
      {cartOpen && (
        <div className="bg-[#130e1d] border border-purple-400/10 rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
              ...
              {cartItems.map((item) => (
                <div key={item._id} className="flex items-center gap-4 bg-white/5 rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-amber-400 text-xs">${item.product?.price?.toFixed(2)}{item.size ? ` / ${item.size}` : ""}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQty({ itemId: item._id, quantity: item.quantity - 1 })} className="w-7 h-7 rounded bg-white/5 text-white/60 hover:text-white flex items-center justify-center text-sm">-</button>
                    <span className="text-white text-sm w-6 text-center">{item.quantity}</span>
                    <button onClick={() => updateQty({ itemId: item._id, quantity: item.quantity + 1 })} className="w-7 h-7 rounded bg-white/5 text-white/60 hover:text-white flex items-center justify-center text-sm">+</button>
                  </div>
                  <button onClick={() => removeItem({ itemId: item._id })} className="text-red-400/60 hover:text-red-400 text-xs">Remove</button>
                </div>
              ))}
              <div className="flex items-center justify-between pt-4 border-t border-white/10">
                <span className="text-white font-semibold">Total: ${cartTotal.toFixed(2)}</span>
                <button className="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-amber-500 hover:from-purple-700 hover:to-amber-600 text-white font-semibold rounded-xl transition-colors">Checkout</button>
              </div>
            </>
          )}
        </div>
      ...
      {!products ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((product) => (
            <div
              key={product._id}
              className={`group bg-[#130e1d] border rounded-2xl overflow-hidden transition-all hover:border-purple-400/30 ${
                product.featured ? "border-amber-400/20 ring-1 ring-amber-400/10" : "border-white/5"
              }`}
            >
              {product.featured && (
                <div className="bg-amber-500/20 text-amber-300 text-xs font-semibold text-center py-1.5 tracking-wider uppercase">Featured</div>
              )}
              <div className="h-40 bg-gradient-to-br from-purple-500/5 to-amber-500/10 flex items-center justify-center text-6xl">
                {product.imageEmoji}
              </div>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                      addedProductId === product._id
                        ...
                        : product.inStock
                        ? "bg-purple-500/15 text-purple-400 border border-purple-400/20 hover:bg-purple-500/25"
      ...
      <div className="text-center py-6 text-white/20 text-xs">
        Independent gear for independent minds. Powered by SwingVote.
      </div>
    </div>
  );
}
