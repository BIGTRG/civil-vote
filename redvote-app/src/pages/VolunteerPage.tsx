          ...
          {(["leaderboard", "shifts", "signup"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={"px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all " +
                (tab === t ? "bg-[#D22B2B] text-white" : "text-white/40 hover:text-white/70 hover:bg-white/5")
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="w-24 h-1.5 bg-white/10 rounded-full mt-1 overflow-hidden">
                      <div className="h-full bg-[#D22B2B] rounded-full" style={{ width: ((s.totalSpots - s.spotsLeft) / s.totalSpots * 100) + "%" }} />
                    </div>
                  </div>
                </div>
                <button className={"mt-3 w-full py-2 rounded-lg text-sm font-medium transition-all " +
                  (s.spotsLeft > 0 ? "bg-[" + "#D22B2B" + "] hover:opacity-90 text-white" : "bg-white/5 text-white/30 cursor-not-allowed")
        ...
        {tab === "signup" && (
          <div className="bg-white/5 border border-white/10 rounded-xl p-6 max-w-lg mx-auto">
            ...
            <form className="space-y-4" onSubmit={e => { e.preventDefault(); alert("Volunteer signup submitted!"); }}>
              ...
              <button type="submit" className="w-full py-3 rounded-lg font-medium text-white transition-all hover:opacity-90" style={{ backgroundColor: "#D22B2B" }}>
