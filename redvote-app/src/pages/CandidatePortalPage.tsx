  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-red-300/60 mt-1">Get your campaign on CivilVote's platform</p>
      </div>

      {/* Hero */}
      <div className="p-8 rounded-2xl bg-gradient-to-br from-red-500/20 to-red-800/10 border border-red-500/20 text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-white">The Most Advanced Campaign Technology Platform</h2>
        <p className="text-red-200/60 mt-3 max-w-2xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {tiers.map(tier => (
          <div key={tier.name} className={`p-6 rounded-2xl border ${tier.popular ? "bg-red-500/10 border-red-500/30 ring-1 ring-red-500/20" : "bg-white/5 border-white/10"}`}>
            {tier.popular && (
              <span className="px-3 py-0.5 bg-red-500 text-white text-xs font-bold rounded-full uppercase tracking-wider">Most Popular</span>
            )}
            ...
            <button className={`w-full mt-6 py-3 rounded-xl font-semibold transition-colors ${tier.popular ? "bg-red-500 text-white hover:bg-red-600" : "bg-white/10 text-white hover:bg-white/15"}`}>
              Contact Sales
            </button>
          </div>
        ))}
      ...
      {/* Contact CTA */}
      <div className="p-8 rounded-2xl bg-gradient-to-r from-red-600/20 to-red-500/10 border border-red-500/20 text-center">
        <h2 className="text-2xl font-bold text-white">Ready to Win Your Race?</h2>
        <p className="text-red-200/50 mt-2">Contact our team to get your campaign on CivilVote.</p>
        <div className="mt-4 text-red-400 font-medium">sales@civilvotetechnologies.com</div>
      </div>
    </div>
  );
}
