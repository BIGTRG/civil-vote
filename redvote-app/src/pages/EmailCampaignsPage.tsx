      ...
      {activeTab === "campaigns" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Subscribers", value: totalSubs.toLocaleString(), color: "text-red-400" },
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              ...
              <button onClick={() => setActiveTab("compose")} className="bg-red-600 hover:bg-red-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">New Campaign</button>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>{mockCampaigns.map(c => (
                  <tr key={c.id} className="border-t border-white/5 hover:bg-white/5 cursor-pointer">
                    ...
                    <td className="px-4 py-3"><span className={"px-2 py-1 rounded-full text-xs " + ({ active: "bg-green-500/20 text-green-400", sent: "bg-red-500/20 text-red-400", draft: "bg-white/10 text-white/50", scheduled: "bg-amber-500/20 text-amber-400" }[c.status] || "")}>{c.status}</span></td>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" placeholder="e.g., July Fundraising Push" className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-red-400" />
            </div>
            <div>
          ...
          <div>
            ...
            <input type="text" placeholder="e.g., We need your help today" className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-red-400" />
          </div>
          <div>
            <label className="block text-sm text-white/50 mb-1">Email Body</label>
            <textarea placeholder="Write your email content here. Use {firstName}, {state}, {donateUrl} for merge fields..." className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-red-400 h-48 resize-none" />
          </div>
          <div className="flex gap-3 pt-2">
            <button className="bg-red-600 hover:bg-red-500 text-white px-6 py-3 rounded-lg font-semibold transition-colors">Send Now</button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {mockSegments.map(seg => (
            <div key={seg.name} className="bg-white/5 border border-white/10 rounded-xl p-5 hover:border-white/20 cursor-pointer transition-colors">
              ...
              <p className="text-2xl font-bold text-red-400">{seg.count.toLocaleString()}</p>
              <p className="text-xs text-white/40 mt-1">contacts in segment</p>
            </div>
          ))}
          <div className="bg-white/5 border border-dashed border-white/20 rounded-xl p-5 flex items-center justify-center cursor-pointer hover:border-red-400 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {templates.map(t => (
            <div key={t.name} className="bg-white/5 border border-white/10 rounded-xl p-5 hover:border-white/20 cursor-pointer transition-colors">
              ...
              <p className="text-sm text-red-400 mb-2">{t.subject}</p>
              <p className="text-xs text-white/40">{t.preview}</p>
              <button className="mt-3 text-sm text-red-400 hover:text-red-300">Use Template</button>
            </div>
          ))}
        </div>
      )}
