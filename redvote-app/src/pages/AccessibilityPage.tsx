      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="range" min={75} max={200} step={25} value={fontSize} onChange={e => setFontSize(Number(e.target.value))} className="w-full accent-red-500" />
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {["normal","relaxed","loose"].map(s => (
                    <button key={s} onClick={() => setLineSpacing(s)} className={"flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-colors " + (lineSpacing === s ? "bg-red-600 text-white" : "bg-white/10 text-white/50")}>{s.charAt(0).toUpperCase() + s.slice(1)}</button>
                  ))}
                </div>
              </div>
              {[
                ...
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <button onClick={item.toggle} className={"w-10 h-6 rounded-full transition-colors relative " + (item.state ? "bg-red-600" : "bg-white/20")}>
                    <div className={"w-4 h-4 rounded-full bg-white absolute top-1 transition-transform " + (item.state ? "translate-x-5" : "translate-x-1")} />
                  </button>
                </div>
              ))}
            </div>
            <button className="w-full mt-4 bg-red-600 hover:bg-red-500 text-white py-2.5 rounded-lg text-sm font-medium transition-colors">Save Settings</button>
          </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mt-4">
            ...
            <textarea placeholder="Describe the accessibility issue you encountered..." className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-red-400 h-24 resize-none mb-3" />
            <button className="bg-red-600 hover:bg-red-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">Submit Report</button>
          </div>
        </div>
      </div>
    </div>
