...
function getSessionId() {
  let id = sessionStorage.getItem("rv_cart_session");
  if (!id) {
    id = crypto.randomUUID();
    sessionStorage.setItem("rv_cart_session", id);
  }
  return id;
}
...
export function StorePage() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">
            Red<span className="text-red-400">Store</span>
          </h1>
          <p className="text-white/50 mt-1">Official RedVote merchandise -- wear your values, show your strength</p>
        </div>
        <button
          onClick={() => setCartOpen(!cartOpen)}
          className="relative flex items-center gap-2 px-4 py-2.5 bg-red-500/15 hover:bg-red-500/25 border border-red-400/20 rounded-xl text-red-400 transition-all"
        >
      ...
      {/* Cart drawer */}
      {cartOpen && (
        <div className="bg-[#1a0a0a] border border-red-400/10 rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <>
              {cartItems.map((item) => (
                <div key={item._id} className="flex items-center gap-4 bg-white/5 rounded-xl p-3">
                  <div className="w-12 h-12 bg-red-500/10 rounded-lg flex items-center justify-center text-2xl">
                    {item.product?.imageEmoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{item.product?.name}</p>
                    <p className="text-red-400 text-xs">${item.product?.price?.toFixed(2)}{item.size ? ` / ${item.size}` : ""}</p>
                  </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/10">
                ...
                <button className="px-6 py-2.5 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl transition-colors">Checkout</button>
              </div>
            </>
          )}
        </div>
      ...
      {!products ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((product) => (
            <div
              key={product._id}
              className={`group bg-[#1a0a0a] border rounded-2xl overflow-hidden transition-all hover:border-red-400/30 ${
                product.featured ? "border-red-400/20 ring-1 ring-red-400/10" : "border-white/5"
              }`}
            >
              {product.featured && (
                <div className="bg-red-500/20 text-red-300 text-xs font-semibold text-center py-1.5 tracking-wider uppercase">Featured</div>
              )}
              <div className="h-40 bg-gradient-to-br from-red-500/5 to-red-600/10 flex items-center justify-center text-6xl">
                {product.imageEmoji}
              </div>
      ...
      <div className="text-center py-6 text-white/20 text-xs">
        Every purchase is a walking advertisement for American values. Powered by RedVote.
      </div>
    </div>
  );
}
