...
export function InstallPrompt() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-[#0f0a0a] border-t border-white/10 shadow-2xl safe-bottom md:hidden">
      <div className="max-w-md mx-auto flex items-center gap-3">
        <img src="/icons/icon-72.png" alt="App" className="w-12 h-12 rounded-xl flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="text-white font-semibold text-sm">Install RedVote</p>
          <p className="text-white/50 text-xs">
            {showIOSPrompt ? 'Tap Share, then "Add to Home Screen"' : "Add to your home screen for the full experience"}
          </p>
        </div>
        {deferredPrompt && (
          <button onClick={handleInstall} className="px-4 py-2 bg-red-600 text-white text-sm font-bold rounded-lg flex-shrink-0 hover:bg-red-500 transition-colors">
            Install
          </button>
        )}
